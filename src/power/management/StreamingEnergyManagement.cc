...
#include "StreamingEnergyManagement.h"
...
Define_Module(StreamingEnergyManagement);

StreamingEnergyManagement::~StreamingEnergyManagement() {
    cancelAndDelete(lifecycleOperationTimer);
}

void StreamingEnergyManagement::initialize(int stage) {
    if (stage == INITSTAGE_LOCAL) {
        ...
        energyStorage = check_and_cast<IEpEnergyStorage *>(
                networkNode->getSubmodule("energyStorage"));
        auto energyStorageModule = check_and_cast<cModule *>(energyStorage);
        energyStorageModule->subscribe(
                IEpEnergySource::powerConsumptionChangedSignal, this);
        energyStorageModule->subscribe(
                IEpEnergySink::powerGenerationChangedSignal, this);
        nodeStatus = dynamic_cast<NodeStatus *>(networkNode->getSubmodule(
                "status"));
        if (!nodeStatus)
            throw cRuntimeError("Cannot find node status");
        lifecycleController =
                dynamic_cast<LifecycleController *>(getModuleByPath(
                        "lifecycleController"));
...
void StreamingEnergyManagement::handleMessage(cMessage *message) {
...
void StreamingEnergyManagement::executeNodeOperation(J estimatedEnergyCapacity) {
    if (!std::isnan(nodeShutdownCapacity.get())
            && estimatedEnergyCapacity <= nodeShutdownCapacity
    ...
    } else if (!std::isnan(nodeStartCapacity.get())
            && estimatedEnergyCapacity >= nodeStartCapacity
            && nodeStatus->getState() == NodeStatus::DOWN) {
...
void StreamingEnergyManagement::scheduleLifecycleOperationTimer() {
...
J StreamingEnergyManagement::getEstimatedEnergyCapacity() const {
...
void StreamingEnergyManagement::receiveSignal(cComponent *source,
        simsignal_t signal, double value, cObject *details) {
    Enter_Method("receiveSignal");
    if (signal == IEpEnergySource::powerConsumptionChangedSignal
            || signal == IEpEnergySource::CPUPowerConsumptionChangedSignal
            || signal == IEpEnergySource::WirelessPowerConsumptionChangedSignal
            || signal == IEpEnergySink::powerGenerationChangedSignal) {
        executeNodeOperation(getEstimatedEnergyCapacity());
        scheduleLifecycleOperationTimer();
    }
}
